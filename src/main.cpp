#include <iostream> <grpcpp/grpcpp.h> <volt_guard.grpc.pb.h>
class VGImpl final:public voltguard::VoltGuard::Service{
    ...
    grpc::Status GetSensors(grpc::ServerContext*,const voltguard::Empty*,voltguard::SensorData*resp)override{
        resp->set_temp_celsius(45);resp->set_voltage_mv(12000);resp->set_current_ma(800);resp->set_power_mw(9600);return grpc::Status::OK;}
...
int main(){VGImpl svc;grpc::ServerBuilder b;b.AddListeningPort("0.0.0.0:50055",grpc::InsecureServerCredentials());b.RegisterService(&svc);auto s=b.BuildAndStart();std::cout<<"Volt Guard on :50055\n";s->Wait();}
